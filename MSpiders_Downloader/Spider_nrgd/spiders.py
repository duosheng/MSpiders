...
import time
from MSpiders_Downloader.settings import Spider_nrgd as settings
...
class Spider_nrgd(CrawlerRequests):

    def crawl0(self, seed_packed):
        """
        html_packed键:
            'parser'(必选): 调用哪个解析规则, 格式为'Parser_nrgd.parse1' 或 'Parser_nrgd'(默认使用parse0方法)。
            'html'(必选): 待解析的HTML。
        """
        html_packed = {'html': None, 'parser': 'Parser_nrgd.parse0'}
        failure = 0
        while failure < 3:
            time.sleep(settings.get('DELAY', 2))
            proxy = getOneProxy(keyName='Proxies:nrgd', obj=self) if settings.get('ISPROXY', False) else None
            response = self.visit(seed_packed['url'], proxy=proxy, retry=settings.get('RETRY', 2))
            if response is None:
                failure += 1
            elif response['status_code'] not in [403, ]:
                if proxy is not None:
                    putOneProxy(proxy=proxy, keyName='Proxies:nrgd', obj=self)
