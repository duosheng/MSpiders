...
class Parser_gdeegd(object):
    ...
    def parse0(self, html_packed):
        ...
        seed键:
            'url'(必选): 待爬URL。
            'spider'(必选): 调用哪个下载规则, 格式为'Spider_gdeegd.crawl1' 或 'Spider_gdeegd'(默认使用crawl0方法)。
        field键:
        ...
        try:
            ...
            data = {}
            url = html_packed['url']
            data['spider'] = '广东省生态环境厅'
            data['url'] = url
            data['type'] = '1'
            data['title'] = selector.xpath('/html/body/div[1]/div/div[2]/div[2]/div[2]/p/text()').extract()[0]
            data['source'] = selector.xpath('//*[@id="docShare"]/div[1]/span[2]/text()').extract()[0].replace("来源  :  ", '')
            data['pushTime'] = selector.xpath('//*[@id="docShare"]/div[1]/span[1]/text()').extract()[0]
            page = selector.xpath('/html/body/div[1]/div/div[2]/div[2]/div[5]').extract()[0]
            files = []
            for at in selector.xpath('/html/body/div[1]/div/div[2]/div[2]/div[5]//a'):
