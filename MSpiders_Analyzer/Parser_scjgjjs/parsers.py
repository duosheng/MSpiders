...
class Parser_scjgjjs(object):
    ...
    def parse0(self, html_packed):
        ...
        seed键:
            'url'(必选): 待爬URL。
            'spider'(必选): 调用哪个下载规则, 格式为'Spider_scjgjjs.crawl1' 或 'Spider_scjgjjs'(默认使用crawl0方法)。
        field键:
        ...
        try:
            ...
            url = html_packed['url']
            data['url'] = url
            data['spider'] = '江苏省市场监督管理局'
            data['type'] = '1'
            data['title'] = selector.xpath('//*[@id="barrierfree_container"]/div[5]/div/p/text()').extract()[0]
            # data['source'] = selector.xpath('//span[@class="ly"]/text()').extract()[0].replace("来源  :  ", '')
            data['pushTime'] = selector.xpath('//*[@id="barrierfree_container"]/div[5]/div/div/div[1]/span[1]/text()').extract()[0].replace(
                "发布日期：", '')[0:10]
            page = selector.xpath('//div[@class="main-txt"]').extract()[0]
            files = []
            for at in selector.xpath('//div[@class="main-txt"]//a'):
