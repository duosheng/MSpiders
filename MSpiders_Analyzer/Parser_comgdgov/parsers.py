...
class Parser_comgdgov(object):
    ...
    def parse0(self, html_packed):
        ...
        seed键:
            'url'(必选): 待爬URL。
            'spider'(必选): 调用哪个下载规则, 格式为'Spider_comgdgov.crawl1' 或 'Spider_comgdgov'(默认使用crawl0方法)。
        field键:
        ...
        try:
            ...
            data['url'] = url
            data['type'] = '1'
            data['title'] = selector.xpath('/html/body/div[2]/div[1]/div[2]/h1/text()').extract()[0]
            data['source'] = selector.xpath('/html/body/div[2]/div[1]/div[2]/div[1]/div[1]/text()').extract()[0]
            data['pushTime'] = selector.xpath('/html/body/div[2]/div[1]/div[2]/div[1]/div[1]/text()').extract()[0].replace(
                "&nbsp;", '')[-19:-9]
            page = selector.xpath('//div[@class="cont-zt"]').extract()[0]
            files = []
            for at in selector.xpath('//div[@class="cont-zt"]//a'):
